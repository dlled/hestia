...
| --- | --- | --- | --- |
| IAM 001–008 | Implemented PoC | Encore `identity` service plus browser owner bootstrap, discoverable WebAuthn passkeys, one-time guest invitations, hashed sessions, scoped expiring grants, recovery codes, administration/revocation and audit; Chrome black-box uses two isolated authenticators | Service-account lifecycle, time-window/multi-home administration and a second physical browser-engine acceptance pass |
...
| UI 001–010 | Partial | dashboard, sleep preview, approval, cancellation, workflow/audit display, passkey identity and identity administration | full realtime UX, automation builder/simulator, preferences/channels, notifications and degraded/offline states |
| NOT 001–007 | Partial | Encore incident persistence, local inbox delivery, deduplication, scoped acknowledgement/escalation and audit | preferences, external channels and Temporal timer-driven escalation |
